...
class cruiseControl:
            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses:
                    ...
                    if global_vaild_object[i][0] == 0:
                        ...
                        dis = sqrt(
                            pow(path.pose.position.x - global_vaild_object[i][1], 2)
                    ...
                    if global_vaild_object[i][0] == 3:
                        ...
                        if traffic_sign == "STOP":
                            dis = sqrt(
                                pow(path.pose.position.x - global_vaild_object[i][1], 2)
                                + pow(
                                    path.pose.position.y - global_vaild_object[i][2], 2
                                )
                            )
    ...
    def acc(self, local_vaild_object, ego_vel, target_vel, status_msg):
        ...
        if self.Person[0] == True:
            print("ACC ON_person")
            Pedestrian = [
                local_vaild_object[self.Person[1]][1],
                local_vaild_object[self.Person[1]][2],
                local_vaild_object[self.Person[1]][3],
            ]
            time_gap = 0.6
            default_space = 1
        ...
        if self.traffic[0] == True:
            print("Traffic_ON")
            front_vehicle = [
                local_vaild_object[self.traffic[1]][1],
                local_vaild_object[self.traffic[1]][2],
                local_vaild_object[self.traffic[1]][3],
            ]
            time_gap = 0.3
            default_space = 0.1
            dis_safe = ego_vel * time_gap + default_space
                ...
                if obj[0] == 2 or obj[0] == 1:
                    for path_num in range(len(out_path)):
                        ...
                        for path_pos in out_path[path_num].poses:
                            ...
                            dis = sqrt(
                                pow(obj[1] - path_pos.pose.position.x, 2)
                                + pow(obj[2] - path_pos.pose.position.y, 2)
                            )
