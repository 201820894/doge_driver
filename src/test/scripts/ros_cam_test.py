...
# import sys
# sys.path.append(r'/home/yuwongyun/catkin_ws/devel/lib')
import rospy
...
class camera_sim:
    ...
    def callback(self, data):
        ...
        cvt_img = cv2.cvtColor(
            cv_image, cv2.COLOR_RGB2HLS
        )  # color convert hue, lightness, saturation  //COLOR_BGR2HSV:: same result
        h, s, v = cv2.split(255 - cvt_img)  # split to hue, saturation, value??
        lower = (0, 0, 0)
        upper = (255, 255, 255)
        filtered_img = cv2.inRange(cvt_img, lower, upper)  # why?
        ...
        src = np.float32(
            [
                [0, height],
                [width, height],
                [width * 25 / 100, height * 70 / 100],
                [width * 75 / 100, height * 70 / 100],
            ]
        )
        dst = np.float32(
            [
