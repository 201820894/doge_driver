...
class rpScanReceiver:
    def __init__(self):
        ...
        self.sublidar = rospy.Subscriber("/lidar2D1", LaserScan, self.callback)
    ...
    def callback(
        self, data
    ):  # parameter of callback 'data' callback is basic foramt maybe?
        min_angle = data.angle_min
        max_angle = data.angle_max
        min_range = data.range_min
        max_range = data.range_max

        angle_increment = data.angle_increment  # 1 degree maybe
        ...
        for i in range(len(data.ranges)):
            x, y = self.calc_axis_xy(
                min_angle + angle_increment * i, data.ranges[i], min_range, max_range
            )
            if [x, y] != [0, 0]:  # can recognize self
                PC_data.points.append(Point32(x, y, 1))  # basic form**
        # self.f.write(str(x)+'\n')
        self.publidar.publish(PC_data)  # publish data
...
# x, y = self.calc_axis_xy(min_angle+angle_increment*i,data.ranges[i], min_range,max_range) #what does i mean???
# i: 0 to 359, angle_increment: unit(1 degree= 6.28/360 radian=0.0174...)
#
#            calc_axis_xy(self, _theta, _distance, _min_range, _max_range):
#               if _min_range <= _distance <= _max_range:
#                   x = np.cos(_theta) * _distance
#                   y = np.sin(_theta) * _distance
